...
import com.rifaqat.knowledgesharing.databinding.FragmentDocumentBinding;
...
public class DocumentFragment extends Fragment {
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 102 && resultCode == Activity.RESULT_OK) {
            assert data != null;
            docUri = data.getData();
            binding.postImage.setImageResource(R.drawable.document_pic);
    ...
    private void ProcessVideoUploading() {
        dialog.show();
        StorageReference reference = storageReference.child("documents")
                .child(Objects.requireNonNull(FirebaseAuth.getInstance().getUid()))
            ...
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                reference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    ...
                    public void onSuccess(Uri uri) {
                        ...
                        document.setPostDocument(uri.toString());
                        document.setPostedDocumentBy(FirebaseAuth.getInstance().getUid());
                        document.setTitle(binding.description.getText().toString());
                        document.setPostedVideoAt(new Date().getTime());
                        databaseReference
                                .push()
                                .setValue(document).addOnSuccessListener(new OnSuccessListener<Void>() {
